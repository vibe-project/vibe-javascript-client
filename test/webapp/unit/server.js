var url = "http://" + (typeof location !== "undefined" ? location.hostname : "localhost") + ":8080/test",
	...
	isCrossDomain = !new RegExp("^" + react.support.getAbsoluteURL("/")).test(react.support.getAbsoluteURL(url)), 
	text2KB = (function() {
		var i, text = "A";
...
helper.each({
	...
	streamxhr: {
		can: typeof XMLHttpRequest !== "undefined" && 
			(!react.support.browser.msie || +react.support.browser.version.split(".")[0] > 9) && 
				(!isCrossDomain || react.support.corsable)
	},
	streamxdr: {
		can: typeof XDomainRequest !== "undefined",
		setup: function() {
			react.defaults.xdrURL = function(url) {
				return url;
			};
		},
		test: function() {
			test("transprot should be skipped if xdrURL is false", 1, function() {
				react.open(url, {xdrURL: false}).close(function(reason) {
	...
	longpollajax: {
		can: isNode || (!isCrossDomain || react.support.corsable)
	},
	longpollxdr: {
		can: typeof XDomainRequest !== "undefined",
		setup: function() {
	...
	QUnit.module(transport, {
		setup: function() {
			helper.setup();
			react.defaults.transports = [transport];
			react.defaults.notifyAbort = true;
			if (group.setup) {
				group.setup();
			}
		},
...
function transportTest() {
	...
	asyncTest("transport should be able to exchange an event", 1, function() {
		react.open(url).on("echo", function(data) {
	...
	// transport itself, but are provided to help to implement the react server
	
	asyncTest("transport should support reply by server", 2, function() {
		var ready;
		
		react.open(url).send("reply-by-server", true, function(echo) {
	...
	asyncTest("transport should support reply by client", 1, function() {
		react.open(url).on("reply-by-client", function(data, reply) {
			ok(true);
			reply("echo");
		})
		.on("echo", function() {
	...
	asyncTest("transport should support heartbeat", 5, function() {
		...
		react.open(url, {heartbeat: 1500, _heartbeat: 500}).on("heartbeat", function() {
			i++;
			ok(true);
			if (i > 4) {
				start();
